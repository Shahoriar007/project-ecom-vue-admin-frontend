<template>
  <b-card>
      ...
      <div class="custom-search d-flex align-items-center justify-content-end">
        ...
        >
          ...
          <template
            v-if="
              $permissionAbility(
                permissionsConstant.DIVISION_CREATE,
                permissions
              )
            "
          >
            ...
            <span>
              ...
              >
                ...
                <template
                  v-if="
                    $permissionAbility(
                      permissionsConstant.DIVISION_EDIT,
                      permissions
                    )
                  "
                >
                ...
                <template
                  v-if="
                    $permissionAbility(
                      permissionsConstant.DIVISION_DELETE,
                      permissions
                    )
                  "
                >
    ...
    <b-modal
      ...
      centered
      :title="modelType == 'editModel' ? 'Edit Division' : 'Create Division'"
      hide-footer
      @hidden="hiddenModal"
...
export default {
  ...
  async created() {
    try {
      const [users] = await Promise.all([this.getUsers()]);
  ...
  methods: {
    ...
    async onDelete(id) {
      try {
        await this.$api.delete(`/api/divisions/${id}`);
    ...
    async loadItems() {
      try {
        ...
        const meta = divisions?.data?.meta;
